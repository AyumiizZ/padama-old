    ...
    <v-dialog v-model="addPatientDialog" width="500">
      ...
      <v-card>
        ...
        <v-card-text v-if="page == 2">
          ...
          <vc-date-picker v-model="operativeDate" v-if="!noneOperation">
            <template v-slot="{ inputValue, inputEvents }">
              <v-text-field
                readonly
...
export default {
  ...
  computed: {
    ...
    operativeDateErrors() {
      const errors = [];
      if (!this.$v.operativeDate.$dirty) return errors;
      !this.$v.operativeDate.required && errors.push("This is required");
      return errors;
    },
    onsetErrors() {
      const errors = [];
  ...
  methods: {
    ...
    async createPatient() {
      try {
        var caseBody;
        console.log(this.noneOperation);
